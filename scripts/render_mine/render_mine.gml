...
function render_mine(_camera_x, _camera_y, _camera_width, _camera_height, _mining_current)
{
    ...
    draw_sprite_ext(_data.sprite, _tile.get_index() + _tile.get_index_offset(), _xstart, _ystart, _xscale, _yscale, 0, c_white, 1);
    
    gpu_set_colorwriteenable(true, true, true, false);
    
    static __index_max = (sprite_get_number(spr_Animation_Mine) - 1);
    
    var _index = round(_progress * __index_max);
    ...
    for (var i = -_width; i <= _width; ++i)
    {
        ...
        for (var j = -_height; j <= _height; ++j)
        {
            ...
            draw_sprite_ext(spr_Animation_Mine, _index, _x, _y, 1, 1, 0, c_white, 1);
        }
    }
