function gui_effects()
{
	...
	for (var i = 0; i < _effect_length; ++i)
	{
		...
		var _x = GUI_SAFE_ZONE_X + (INVENTORY_LENGTH.ROW * INVENTORY_SLOT_WIDTH * INVENTORY_SLOT_SCALE) + ((_effect_width + GUI_EFFECT_XOFFSET) * GUI_EFFECT_SCALE * (_index %  GUI_EFFECT_ROW)) + (GUI_EFFECT_XOFFSET * GUI_EFFECT_SCALE) + (_effect_width * GUI_EFFECT_SCALE / 2);
		var _y = GUI_SAFE_ZONE_Y + (INVENTORY_SLOT_HEIGHT * INVENTORY_SLOT_SCALE / 2) + ((_effect_height + GUI_EFFECT_YOFFSET) * GUI_EFFECT_SCALE * floor(_index / GUI_EFFECT_ROW));
		...
		if (point_in_rectangle(_mx, _my, _x - GUI_EFFECT_HOVER_OFFSET, _y - GUI_EFFECT_HOVER_OFFSET, _x + GUI_EFFECT_HOVER_OFFSET, _y + GUI_EFFECT_HOVER_OFFSET))
		{
			...
			draw_text(_mx, _my, $"{_loca} {_effect.level} ({_time})");
		}
	}
}