...
function init_structure(_directory, _prefix = "phantasia", _type = 0)
{
    ...
    for (var i = 0; i < _files_length; ++i)
    {
        ...
        global.structure_data[$ _name] = array_create(_files2_length);
        ...
        for (var j = 0; j < _files_length; ++j)
        {
            ...
            if (string_ends_with(_file, ".json"))
            {
                if (!file_exists($"{_directory}/{_file}/{string_delete(_file2, string_length(_file2) - 4, 5)}.dat"))
                {
                    global.structure_data[$ _name][@ j] = __init_json(_prefix, _file, $"{_directory}/{_file}/{_file2}");
                }
            ...
            if (string_ends_with(_file, ".dat"))
            {
                global.structure_data[$ _name][@ j] = __init_dat(_prefix, _file, $"{_directory}/{_file}/{_file2}", _item_data);
            }
        }
    }
}